"""
Behavioral Cloning Training Script — PnPCounterToCab (RoboCasa)
===============================================================
Trains a supervised MLP on robocasa LeRobot-format demonstration data.

Observation space (16D, robot proprioception only — no object state):
...
Action space (12D, HDF5/env ordering):
  ...
  (Dataset stores actions in LeRobot ordering; bc_dataset.py reorders at load time.)

Usage
-----
python RoboCasa_Code_Aide/my_rl_scripts/train_bc.py \
...
from my_env import MyPnPCounterToCab, BCAgent
from my_env.bc_dataset import BCDemoDataset
from my_env.bc_policy import extract_bc_obs
...
def evaluate(agent: BCAgent, raw_env, n_episodes: int, horizon: int):
    """
    Roll out the BC agent in the environment using 16D proprioceptive obs.
    Returns (success_rate, mean_reward).
    """
    successes, rewards = [], []

    for ep in range(n_episodes):
        # Seed is set via env constructor; reset with different object positions
...
def main():
    parser = argparse.ArgumentParser(description="Train BC on RoboCasa demos")
    parser.add_argument("--config",      type=str, required=True)
    parser.add_argument("--dataset_dir", type=str, default=None,
                        help="Override dataset directory from CLI")
    ...
    if dataset.obs_dim == 16:
        print(
            "[WARNING] This BC setup only sees 16D robot proprioception. "
    ...
    # ------------------------------------------------------------------
    # Environment (only used for evaluation)
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    best_success = -1.0
    print(f"\n[INFO] Training BC for {t_cfg['epochs']} epochs on {device}")
    ...
    for epoch in range(1, t_cfg["epochs"] + 1):
        ...
        for obs_b, act_b in train_loader:
            ...
            loss = loss_fn(agent.net(obs_norm), act_b)
            optimizer.zero_grad()
            loss.backward()
        ...
        with torch.no_grad():
            for obs_b, act_b in val_loader:
                obs_norm = (obs_b.to(device) - obs_mean_t) / (obs_std_t + 1e-8)
                val_losses.append(loss_fn(agent.net(obs_norm), act_b.to(device)).item())

        train_loss = float(np.mean(train_losses))
        val_loss   = float(np.mean(val_losses))
        metrics    = {"bc/train_loss": train_loss, "bc/val_loss": val_loss}
        ...
        if epoch % t_cfg["eval_every"] == 0 or epoch == t_cfg["epochs"]:
            ...
            if success_rate > best_success:
                ...
                agent.save(os.path.join(save_path, "bc_best.pt"))
    ...
    # Final save
    # ------------------------------------------------------------------
    final_path = os.path.join(save_path, "bc_final.pt")
    agent.save(final_path)
    print(f"\n[INFO] Final model → {final_path}")
    print(f"[INFO] Best model (success={best_success:.2f}) → {save_path}/bc_best.pt")
    if use_wandb:
        wandb.finish()
