"""
Evaluation script for image-conditioned BC.
"""
...
from my_env import ImageBCAgent, MyPnPCounterToCab
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate an image-conditioned BC model")
    ...
    agent = ImageBCAgent.load(args.model_path)
    camera_names = sorted(set(agent.camera_names) | set(VIZ_CAMERAS if args.save_video else []))
    ...
    raw_env = MyPnPCounterToCab(
        ...
        camera_heights=agent.image_size,
        camera_widths=agent.image_size,
        seed=args.seed,
    )
    ...
    print(
        f"  Success rate: {success_count}/{args.episodes} "
